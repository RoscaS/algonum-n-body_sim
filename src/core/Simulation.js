...
function init() {
  ...
  newPlanetSettings
    .add(_settings, "mass", _minMass, _maxMass)
    .step(1000)
    .name("Mass")
    .listen();
  newPlanetSettings
    .addColor(_settings, "color")
    .name("Color")
    .listen();
  newPlanetSettings.open();
  var globalSettings = gui.addFolder("Global Controls");
  globalSettings
    .add(_settings, "shouldClear")
    .name("Remove Planets")
    .listen();
  globalSettings
    .add(_settings, "trailOpacity", 0.0, 1.0)
    .step(0.01)
    .name("Trail Opacity")
    .listen();
  globalSettings.add(_settings, "timeStep", 1 / 250, 1 / 5).name("Time Step");
  globalSettings
    .add(_settings, "integration", _integrationValues)
    .name("Integration")
    .listen();
  globalSettings
    .add(_settings, "collisions", _collisionValues)
    .name("Collisions")
    .listen();
  globalSettings
    .add(_settings, "damping", 0, 100)
    .name("Damping")
    .listen();
        ...
        if (!p.collided && !op.collided) {
          ...
          if (d < op.r + p.r) {
            ...
            _collisions.push(
              new Planet(
                totalMass,
                (op.x * op.m + p.x * p.m) / totalMass,
                (op.y * op.m + p.y * p.m) / totalMass,
                (op.vx * op.m + p.vx * p.m) / totalMass,
                (op.vy * op.m + p.vy * p.m) / totalMass,
                rgbToHex(r, g, b)
              )
            );
          }
...
function takeStep(j) {
  ...
  if (_settings.integration == _integrationValues[1]) {
    ...
    p.y += p.vy * h;
    p.oy = py;
  } else if (_settings.integration == _integrationValues[0]) {
    // rk4
    var x1 = ppx;
    var y1 = ppy;
    var vx1 = (1 - f) * ppvx;
    ...
    p.vx =
      (1 - f) * p.vx + (h / 6.0) * (a1[0] + 2.0 * a2[0] + 2.0 * a3[0] + a4[0]);
    p.vy =
      (1 - f) * p.vy + (h / 6.0) * (a1[1] + 2.0 * a2[1] + 2.0 * a3[1] + a4[1]);

    p.x += (h / 6.0) * (vx1 + 2.0 * vx2 + 2.0 * vx3 + vx4);
    p.ox = px;
    p.y += (h / 6.0) * (vy1 + 2.0 * vy2 + 2.0 * vy3 + vy4);
    p.oy = py;
  } else if (_settings.integration == _integrationValues[2]) {
    // verlet
    var a = nsquaredacceleration(j, ppx, ppy, pp.r);
    if (p.isnew) {
      p.ox = p.x;
...
function system(ix, iy, ivx, ivy) {
  ...
  for (var i = 0; i < 300; i++) {
    ...
    var v = Math.min(
      2800.0 * Math.pow(r, -1.0 / 2.0) +
        30 +
        Math.pow((m / _minMass) * 1.3, 2.6),
      1000.0
    );
